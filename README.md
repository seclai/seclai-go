...
Fields are pointers: a nil field is left unchanged, and a pointer to the zero
value clears it.

```go
alias := "support"
senders := []string{"example.com", "ops@partner.com"}
requireAuth := true
...
cfg, _ := client.SetEmailTriggerConfig(ctx, "agent_id", "trigger_id",
    seclai.RoutersApiAgentsSetEmailTriggerConfigRequest{
        ...
        RequireSenderAuth: &requireAuth,
        // IgnoreAutoGenerated and QueueOnQuota left nil — unchanged.
    })
fmt.Println(cfg.EmailAddresses)

// Clear the alias explicitly by pointing at the zero value.
empty := ""
_, _ = client.SetEmailTriggerConfig(ctx, "agent_id", "trigger_id",
    seclai.RoutersApiAgentsSetEmailTriggerConfigRequest{Alias: &empty})
```
